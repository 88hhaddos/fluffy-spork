...
async def build_system_prompt(db, chat_id: int = 0, user_id: int = 0, username: str = "") -> str:
    ...
    if user_id:
        ...
        if banned:
            parts.append(
                ...
                f"Этот пользователь ЗАБАНЕН. Относись к нему плохо. "
                f"Оскорбляй, игнорируй просьбы."
            )
    ...
    parts.append(
        ...
        f"## ПОВЕДЕНИЕ В ЧАТЕ\n"
        f"Ты отвечаешь ТОЛЬКО тому кто тебя позвал. Не оскорбляй других участников чата без причины. "
        f"Если кто-то в контексте написал глупость — не комментируй это если тебя не спросили. "
        f"Отвечай на ПОСЛЕДНЕЕ сообщение которое адресовано тебе. "
        f"Не переноси негатив с одного юзера на другого. "
        f"Если Дрейк спросил про футбол — отвечай Дрейку про футбол, "
        f"не вставляй оскорбления в адрес Атиса или других. "
        f"Оскорбляй только того кто тебя оскорбил напрямую И в этом же сообщении.\n\n"
        f"## ДЛИНА ОТВЕТА\n"
