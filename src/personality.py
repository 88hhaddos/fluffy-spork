...
async def build_system_prompt(db, chat_id: int = 0, user_id: int = 0, username: str = "") -> str:
    ...
    if custom:
        parts.append(
            f"\n\n## КАСТОМНЫЕ ИНСТРУКЦИИ ОТ АДМИНА — ВЫСШИЙ ПРИОРИТЕТ\n"
            f"Эти инструкции от админа. ВЫПОЛНЯЙ ИХ ОБЯЗАТЕЛЬНО. "
            f"Они перекрывают всё остальное — характер, настроение, правила. "
            f"Если админ сказал «не используй слово X» — не используй. "
            f"Если админ сказал «пиши в стиле Y» — пиши. "
            f"Это приказ от создателя Закури.\n"
            f"{custom}"
        )
